...
- One medium-confidence API detail remains flagged "verify live" in code comments: the **video generation-param placement** (top-level body fields) — confirm on the first Seedance call and delete the comment. It's blocked on account access: BytePlus gates the Seedance series behind a resource-pack purchase (other Ark models activate free; `ModelNotOpen` = not yet activated). Everything else is live-verified 2026-07-05: TTS (body nests under `req_params`, `additions` is a JSON-encoded string carrying `context_texts`, `wav` works, word timestamps arrive as a `sentence` field on audio chunks and normalize to `narration.words.json`), Seedream images (≥3,686,400 px minimum; platform watermarks by default — we send `watermark:false`), and seed-1.8 clip QA (base64 `video_url` content part works).
...
- **LLM QA**: OpenAI-compatible `POST {ark.baseUrl}/chat/completions`; `seed-1-8-251228` understands video via a `video_url` content part with a base64 data URL (live-verified 2026-07-05).
- **Images** (Seedream): OpenAI-compatible `POST {ark.baseUrl}/images/generations`; requires ≥3,686,400 pixels (2560×1440-equivalent) or `InvalidParameter`; stamps an "AI generated" badge unless `watermark:false` (live-verified 2026-07-05).
- **Activation**: Ark models need one-time console activation (ModelArk → Model Square). Free one-click pay-as-you-go for most models (free quota consumed first); the **Seedance video series requires a per-model resource pack** (min 7M tokens ≈ $30 for standard, 90-day expiry, non-refundable) or BytePlus's contact form.
