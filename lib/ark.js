...
export async function generateImage(config, key, { prompt, model, size }) {
  const body = { model, prompt, response_format: "b64_json", ...(size ? { size } : {}) };
...
// exact video part shape is the one medium-confidence detail here — verify live).
