...
// Request shape live-verified 2026-07-05 (rendered docs + real calls). Word timestamps
// (enable_timestamp, TTS 1.0/ICL voices only) ride a `sentence` field on audio chunks:
// {phonemes, text, words: [{word, startTime, endTime, confidence}]} — also live-verified.
...
export async function synthesize(config, key, { text, voice, tone, format, sampleRate, wantTimestamps }) {
  ...
  for await (const chunk of parseJsonChunks(stream)) {
    if (chunk.code === 0) {
      ...
      if (chunk.sentence) sentences.push(chunk.sentence);
  ...
  return { audio: Buffer.concat(audioParts), sentences };
}
