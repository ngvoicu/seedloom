...
// Shapes per docs/research-2026-07-byteplus.md; timestamp payload schema is undocumented —
// whatever arrives is preserved raw for the caller (verify live with a TTS 1.0 voice).
...
export async function synthesize(config, key, { text, voice, tone, format, sampleRate, wantTimestamps }) {
  const body = {
    ...
    // TTS 2.0 emotion control is a natural-language prompt (context_texts), not an enum.
    ...(tone ? { context_texts: [tone] } : {}),
  };
