...
const FLAG_SPECS = {
  ...
  image: { valued: ["model", "size"], multi: [], bool: ["json"] },
...
export async function runTts(args) {
  ...
  const { audio, timestamps } = await synthesize(config, key, {
    text,
    voice,
    tone: flags.tone,
  ...
  if (flags.words) {
    if (timestamps.length > 0) {
      // Raw, schema-unverified payload — normalized [{id,text,start,end}] extraction lands
      // after the first live call against a TTS 1.0 voice documents the real shape.
      wordsPath = path.join(dir, "narration.words.raw.json");
      await fs.writeFile(wordsPath, `${JSON.stringify(timestamps, null, 2)}\n`);
    } else {
      wordsNote = `voice "${voice}" returned no native timestamps (TTS 2.0 voices never do) — derive timing externally, e.g. \`npx hyperframes transcribe ${audioPath}\``;
    }
  }
...
export async function runImage(args) {
  ...
  const prompt = flags._[0];
  if (!prompt) throw new Error('Usage: seedloom image "<prompt>" [--model <id>] [--size 2048x2048] [--json]');
  const key = requireKey("ark");
  const { config } = await loadConfig();
  ...
  console.error(`run: ${dir}`);
  const { bytes, usage } = await generateImage(config, key, { prompt, model, size: flags.size });
