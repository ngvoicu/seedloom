...
- **TTS** (Seed Speech): plain-JSON HTTP endpoint `POST {voice.baseUrl}/tts/unidirectional` with headers `X-Api-Key`, `X-Api-Resource-Id` (`seed-tts-2.0`), fixed `X-Api-App-Key: aGjiRDfUWi`; response is streamed JSON chunks with base64 audio (`{code:0,data}…{code:20000000}`) to concatenate. `enable_timestamp` returns word+phoneme timestamps **only on TTS 1.0 / ICL 1.0 voices** — for 2.0 voices derive caption timing externally (e.g. `npx hyperframes transcribe`). TTS 2.0 emotion control = `context_texts` natural-language prompts.
